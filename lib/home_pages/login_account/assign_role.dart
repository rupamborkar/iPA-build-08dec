...
class _AssignRolesScreenState extends State<AssignRolesScreen> {
  ...
  try {
    ...
    if (response.statusCode == 207) {
      ...
      String message = responseData["message"] ?? "Invitation process completed.";

      _showResultDialog(failedList, successList, message);
    } else {
      final responseData = json.decode(response.body);
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(responseData["error"] ?? "Failed to assign roles.")),
      );
    }
  } catch (e) {
...
void _showResultDialog(List failed, List success, String message) {
  ...
  //   final url = Uri.parse('$baseUrl/api/assign_roles');

  //   // Construct the request body with role-email mapping
  //   final body = {
  //     "roles": [
  //       {
  //         "role": "Manager",
  //         "email": managerController.text,
  //       },
  //       {
  //         "role": "Chef",
  //         "email": chefController.text,
  //       },
  //       {
  //         "role": "Other",
  //         "email": otherController.text,
  //       },
  //     ],
  //   };
  ...
  //   final body = {
  //     "manager_email": managerController.text,
  //     "chef_email": chefController.text,
  //     "other_email": otherController.text,
  //   };
