...
import 'package:flutter_app_login/home_pages/menu_edit_pages/edit_menu_detail.dart';
...
class _MenuDetailState extends State<MenuDetail>
              ...
              if (_tabController.index == 0) {
                ...
                if (result == true) {
                  setState(() {
                    ...
                    fetchMenuDetails(); // Example: Fetch updated ingredients
                  });
                }
              } else {
                final result = await Navigator.push(
  ...
  //             onPressed: () {
  //               Navigator.of(context).pop(); // Close the dialog
  //               deleteMenu(widget.menuId); // Delete the menu
  //             },
  //             child: const Text(
  //               'Delete',
  //               style: TextStyle(color: Colors.red),
        ...
        children: [
          Padding(
            ...
            child: Column(
              ...
              children: [
                _buildDetailRow(
                    'Measurement:', '\$${recipe['serving_size'] ?? '0.00'}'),
                _buildDetailRow('Cost:', '\$${recipe['cost'] ?? '0.00'}'),
                _buildDetailRow(
                    'Selling Price:', '\$${recipe['selling_price'] ?? '0.00'}'),
                _buildDetailRow('Food Cost:', '${recipe['food_cost'] ?? '0%'}'),
                _buildDetailRow(
